-- -- DROP FUNCTIONS -- --
DROP FUNCTION IF EXISTS function_remaining_years_to_pof;
DROP FUNCTION IF EXISTS function_remaining_years;
DROP FUNCTION IF EXISTS function_condition_to_life_fraction;

-- -- CREATE FUNCTIONS -- -- 
CREATE OR REPLACE FUNCTION function_condition_to_life_fraction (condition_value numeric, deterioration_id INTEGER)
	RETURNS NUMERIC (3,2)
	LANGUAGE PLPGSQL
	AS
	$$
...
CREATE OR REPLACE FUNCTION function_remaining_years (condition_value numeric, deterioration_id INTEGER, age_value INTEGER, lifespan_value INTEGER, method_text TEXT, inspection_date DATE)
	RETURNS NUMERIC(3,0)
	LANGUAGE PLPGSQL
	AS
	$$
	DECLARE
		...
		life_fraction NUMERIC := function_condition_to_life_fraction(condition_value, deterioration_id);
...
ALTER FUNCTION function_remaining_years(numeric, integer, integer, integer, text, date)
    OWNER TO doadmin
;

CREATE OR REPLACE FUNCTION function_remaining_years_to_pof (years_remaining NUMERIC)
	RETURNS INTEGER
	LANGUAGE PLPGSQL
	AS
	$$
...
ALTER FUNCTION function_remaining_years_to_pof (NUMERIC)
	OWNER TO doadmin
;
